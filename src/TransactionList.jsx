  ...
  return (
    <div style={{ border: '1px solid var(--color-border)' }}>
      <div style={{ borderBottom: '1px solid var(--color-border)', background: 'var(--color-surface)' }}
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          <select value={filterType} onChange={(e) => setFilterType(e.target.value)}
            style={selectStyle}
            className="flex-1 sm:flex-none px-3 py-1.5 text-xs outline-none focus:border-[var(--color-accent)] transition-colors">
          ...
          <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)}
            style={selectStyle}
            className="flex-1 sm:flex-none px-3 py-1.5 text-xs capitalize outline-none focus:border-[var(--color-accent)] transition-colors">
      ...
      {filtered.length === 0 ? (
        ...
        <>
          {/* Mobile card list */}
          <div className="sm:hidden divide-y divide-[var(--color-border)]" style={{ background: 'var(--color-surface-raised)' }}>
            {filtered.map(t => (
              <div key={t.id} className="px-4 py-4 flex items-center gap-3 hover:bg-[var(--color-surface)] transition-colors">
                <div className="flex-1 min-w-0">
                  <p style={{ fontFamily: 'var(--font-display)', color: 'var(--color-text)', fontWeight: 300 }}
                    className="text-lg italic truncate">{t.description}</p>
                  <p style={{ fontFamily: 'var(--font-body)', color: 'var(--color-muted)' }}
                    className="text-xs mt-0.5 uppercase tracking-wide">{t.date} · {t.category}</p>
                </div>
                <span style={{ fontFamily: 'var(--font-display)', color: t.type === 'income' ? 'var(--color-income)' : 'var(--color-expense)', fontWeight: 300 }}
                  className="text-lg shrink-0">
                  {t.type === 'income' ? '+' : '−'}${t.amount.toFixed(2)}
                </span>
                <div className="flex items-center gap-3 shrink-0">
                  <button onClick={() => setEditing({ ...t })} style={{ color: 'var(--color-muted)' }}
                    className="hover:text-[var(--color-income)] transition-colors" title="Edit">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125" />
                    </svg>
                  </button>
                  <button onClick={() => setPendingDelete(t)} style={{ color: 'var(--color-muted)' }}
                    className="hover:text-[var(--color-expense)] transition-colors" title="Delete">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {/* Desktop table */}
          <table className="w-full hidden sm:table" style={{ background: 'var(--color-surface-raised)' }}>
            <thead>
